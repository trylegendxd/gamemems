...
def clear_local_caches() -> dict:
    ...
    Called by the "Limpar Anuncios" admin action to ensure that even when
...
class MarketplaceScraper:
    ...
    # ── Vinted parser ────────────────────────────────────────────────────────
    # Vinted é SPA com REST API JSON. Sessão obtida visitando a homepage 1x.
    # Não requer login do utilizador.

    VINTED_MAX_PAGES = int(os.getenv("VINTED_MAX_PAGES", "3"))
    VINTED_PER_PAGE  = int(os.getenv("VINTED_PER_PAGE", "20"))

    def _vinted_warm_session(self, host: str) -> bool:
        if getattr(self, "_vinted_warmed", None) is None:
            self._vinted_warmed = set()
        if host in self._vinted_warmed:
            return True
        homepage = f"https://{host}/"
        try:
            self.fetch_html(homepage)
            self._vinted_warmed.add(host)
            log.info("[VINTED] sessão aquecida para %s", host)
            return True
        except Exception as e:
            log.warning("[VINTED] falhou warm-up de %s: %s", host, e)
            return False

    def _vinted_to_api_url(self, url: str) -> Optional[str]:
        parsed = urlparse(url)
        ...
        if "per_page=" not in query:
            sep = "&" if query else ""
            query = f"{query}{sep}per_page={self.VINTED_PER_PAGE}"
        return f"https://{parsed.netloc}/api/v2/catalog/items?{query}"

    def _vinted_page_url(self, api_url: str, page: int) -> str:
        if page <= 1:
            return api_url
        sep = "&" if "?" in api_url else "?"
        return f"{api_url}{sep}page={page}"

    def scrape_vinted(self, url: str) -> List["Listing"]:
        api_url = self._vinted_to_api_url(url)
        if not api_url:
            log.warning("[VINTED] URL não reconhecida: %s", url)
            return []

        host = urlparse(api_url).netloc
        if not self._vinted_warm_session(host):
            return []
        ...
        try:
            for page in range(1, self.VINTED_MAX_PAGES + 1):
                page_url = self._vinted_page_url(api_url, page)
                try:
                    body = self.fetch_html(page_url)
                except Exception as e:
                    if page == 1:
                        raise
                    log.info("[VINTED-PAGE] %s page=%d stop (%s)", api_url, page, e)
                    break
                try:
                    data = json.loads(body)
                except json.JSONDecodeError:
                    log.warning("[VINTED-PAGE] %s page=%d resposta não-JSON", api_url, page)
                    break
                items = data.get("items", []) or []
                page_listings = [
                    p for p in (self._parse_vinted_item(it) for it in items) if p is not None
                ]
                new_on_page = 0
                for item in page_listings:
                    key = item.url.split("?")[0]
                    ...
                    new_on_page += 1
                log.info(
                    "[VINTED-PAGE] %s page=%d items=%d new=%d total=%d",
                    api_url, page, len(items), new_on_page, len(listings),
                )
                if new_on_page == 0:
                    break
                if prev_count >= 0 and len(items) < prev_count // 2:
                    break
    ...
    def _parse_vinted_item(self, it: Dict) -> Optional["Listing"]:
        try:
            title = clean_text(it.get("title", "") or "")
            if len(title) < 3:
                return None

            price_field = it.get("price") or it.get("total_item_price")
            price = None
            if isinstance(price_field, dict):
                amount = price_field.get("amount")
                currency = (price_field.get("currency_code") or "EUR").upper()
                if currency != "EUR":
                    return None
                try:
            ...
            if not url:
                ...
                if not item_id:
                    return None
                url = f"https://www.vinted.pt/items/{item_id}"
            if not url.startswith("http"):
                url = urljoin("https://www.vinted.pt", url)

            image_url = ""
            photo = it.get("photo")
            if isinstance(photo, dict):
                image_url = photo.get("full_size_url") or photo.get("url") or ""
                if image_url and image_url.startswith("data:"):
                    image_url = ""

            user = it.get("user") or {}
            location_parts = [user.get("city"), user.get("country_title_local")]
            location = ", ".join(p for p in location_parts if p)

            brand = clean_text(it.get("brand_title", "") or "")
            size = clean_text(it.get("size_title", "") or "")
            base_desc = clean_text(it.get("description", "") or "")
            meta = " · ".join(p for p in [brand, size] if p)
            description = f"{meta}\n\n{base_desc}".strip() if meta else base_desc

            raw_text = clean_text(f"{title} {brand} {size} {base_desc} {location}")[:1000]
            condition = pricing.detect_condition(f"{title} {description}")
            ...
            log.debug("[VINTED] item ignorado: %s", e)
            return None

    # ── Facebook Marketplace parser ──────────────────────────────────────────
    # Requer cookies de sessão via env: FB_C_USER, FB_XS, FB_DATR.
    # Sem cookies devolve [] silenciosamente (com aviso 1x).

    FB_COOKIES_INJECTED = False
    FB_WARNED_NO_COOKIES = False
    FB_MAX_LISTINGS = int(os.getenv("FB_MAX_LISTINGS", "60"))

    def _ensure_fb_cookies(self) -> bool:
        if MarketplaceScraper.FB_COOKIES_INJECTED:
            return True

        c_user = os.getenv("FB_C_USER", "").strip()
        xs = os.getenv("FB_XS", "").strip()
        datr = os.getenv("FB_DATR", "").strip()

        if not (c_user and xs):
            if not MarketplaceScraper.FB_WARNED_NO_COOKIES:
                log.warning(
                    "[FB] FB_C_USER / FB_XS não estão definidos — "
                    "Facebook Marketplace será ignorado."
                )
                MarketplaceScraper.FB_WARNED_NO_COOKIES = True
            return False

        for name, value in [("c_user", c_user), ("xs", xs), ("datr", datr)]:
            if value:
                self.session.cookies.set(
                    name, value, domain=".facebook.com", path="/",
                )
        MarketplaceScraper.FB_COOKIES_INJECTED = True
        log.info("[FB] cookies de sessão injectadas")
        return True

    def scrape_facebook_marketplace(self, url: str) -> List["Listing"]:
        if not self._ensure_fb_cookies():
            return []
        try:
            html = self.fetch_html(url)
        except Exception as e:
            log.warning("[FB] falhou ao buscar %s: %s", url, e)
            return []

        lower_html = html[:5000].lower()
        if "login" in lower_html and "marketplace" not in lower_html[:1000]:
            log.warning(
                "[FB] resposta parece página de login — "
                "cookies expiradas. Refresca FB_C_USER e FB_XS."
            )
            return []

        return self._parse_fb_marketplace_page(html, url)

    _FB_LISTING_RE = re.compile(
    ...
    _FB_ITEM_URL_RE = re.compile(r'/marketplace/item/(\d+)/?')

    def _parse_fb_marketplace_page(self, html: str, source_url: str) -> List["Listing"]:
        listings: List[Listing] = []
        seen_ids: set = set()

        item_ids = list(dict.fromkeys(self._FB_ITEM_URL_RE.findall(html)))
        matches = list(self._FB_LISTING_RE.finditer(html))
        if not matches:
            log.warning(
                "[FB] nenhum listing extraído de %s — markup pode ter mudado",
                source_url,
            )
            return []

        for idx, m in enumerate(matches):
            if len(listings) >= self.FB_MAX_LISTINGS:
                break
            try:
                title_raw = m.group(1)
                title = clean_text(
                    bytes(title_raw, "utf-8").decode("unicode_escape", errors="ignore")
                )
                if len(title) < 3:
                    continue
                price = float(m.group(2))
                currency = m.group(3)
                if currency != "EUR" or price <= 0:
                    continue

                item_id: Optional[str] = None
                for cand in item_ids:
                ...
                if item_id is None:
                    item_url = f"{source_url}#fb-{abs(hash(title)) % (10**9)}"
                else:
                ...
                log.debug("[FB] item idx=%d ignorado: %s", idx, e)
                continue

        log.info(
            "[FB] %s -> %d listings extraídos (de %d candidatos)",
            source_url, len(listings), len(matches),
        )
        return dedupe_listings(listings)
...
MarketplaceScraper.register_source(
    "Vinted",     lambda u: "vinted." in (urlparse(u).netloc or u), "scrape_vinted",
)
MarketplaceScraper.register_source(
    "Facebook",   lambda u: "facebook.com/marketplace" in u or "fb.com/marketplace" in u, "scrape_facebook_marketplace",
)
