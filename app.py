...
def create_app() -> Flask:
    ...
    def api_limpar_anuncios():
        """
        Reset all listing data so the next scan starts fresh.

        Steps:
          1. Refuse if a scan is in progress (race-condition guard).
          2. Hard-delete every row in `deals` and `seen_listings`.
          3. Reset `data/seen.json` and `data/market_cache.json` to `{}`.
          4. Return JSON summary with counts and timestamps.

        Auth: same session-cookie auth as other admin endpoints.
        Confirmation: requires header `X-Confirm-Action: limpar`.
        """
        ...
        try:
            ...
            log.info(
                ...
                db_counts["deals"], db_counts["seen_listings"],
            )
